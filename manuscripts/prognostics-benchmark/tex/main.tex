...
\newcommand{\versiondoi}{10.5281/zenodo.22834808}
\newcommand{\conceptdoi}{10.5281/zenodo.21512482}
\newcommand{\docrepo}{github.com/fsantibanezleal/CAOS\_RotorVitals}
...
CAOS open-research programme \quad$\cdot$\quad RotorVitals, bearing diagnostics and prognostics\\[2pt]
...
\url{https://github.com/fsantibanezleal/CAOS_RotorVitals}; the study is available at
\url{https://rotorvitals.fasl-work.com}.}}

\markboth{\doctype\ \docversion, 2026 \textperiodcentered\ CAOS open-research programme}%
{Santib\'a\~nez-Leal: A leakage-immune bearing diagnostics and prognostics benchmark}
...
four standard public datasets under a leakage-immune protocol and the accepted RUL metric, and reports the
...
leakage-immune by construction reaches recall $1.00$ on healthy and outer-race classes and $0.96$ on inner-race
faults but \emph{fails entirely on ball faults} (recall $0.00$), the hard case. On prognostics (XJTU-SY, FEMTO and IMS run-to-failure
...
is $0.048$ and the best cumulative relative accuracy $0.13$. The low scores are not an implementation error
(the same model tracks a controlled degradation): bearing \emph{diagnostics} is largely solved on these data
when evaluated without leakage, whereas \emph{RUL prediction} on real run-to-failure bearings remains hard at
the accepted metric. Every number and figure regenerates from the archived artifacts, and the raw datasets are
...
This report re-runs both tasks over the four standard public datasets, under protocols chosen to remove those two
...
contribution is not a new method; it is a leakage-immune, reproducible benchmark and the finding it
yields: diagnostics on these data is largely solved, and RUL prediction on real run-to-failure bearings is not.

% Defined here so that the two-column float is placed at the top of page 2, beside Sections II and III.
\begin{figure*}[!t]
\centering
\includegraphics[width=0.9\textwidth]{fig-diagnostics.pdf}
...
leakage-immune unsupervised envelope comb versus the learned WDCNN: the envelope method reaches recall $1.00$,
$1.00$ and $0.96$ on healthy, outer-race and inner-race classes but \emph{fails entirely on ball faults} (recall
$0.00$), the hard case where the ball-fault modulation is smeared across the spectrum; the learned model recovers it.}
...
\section{Diagnostics: Largely Solved, With One Gap}\label{sec:diag}
...
from healthy at an area-under-curve of $1.00$ with a $4.3\%$ false-flag rate on healthy signals. The
tension is in the unsupervised family. The best envelope variant (fixed $2$--$4$~kHz resonance band) reaches
recall $1.00$ on healthy and outer-race signals and $0.96$ on inner-race faults but scores \emph{zero} on ball
faults, and the two other bands are worse; a training-free method that reads fault-frequency harmonics simply
cannot see the ball-fault signature, whose modulation is smeared and non-stationary. In summary,
the unsupervised method is trustworthy \emph{and} leakage-immune on three of four classes and fails on the
...
demodulation bands) and the learned models. Ball faults are the hard case for the unsupervised family.}
\label{tab:diag}
\footnotesize
...
\begin{figure*}[!t]
\centering
\includegraphics[width=0.75\textwidth]{fig-rul-method.pdf}
...
\begin{figure*}[!t]
\centering
\includegraphics[width=0.9\textwidth]{fig-rul-benchmark.pdf}
...
probabilistic models are worse. The low scores reflect the task, not a tuning failure.}
\label{fig:rulbenchmark}
\end{figure*}

The RUL result is Fig.~\ref{fig:rulbenchmark} and Table~\ref{tab:rul}. Across the $23$
...
a single exponential extrapolation cannot track to within $20\%$ at a checkpoint. An accuracy reported as a
trajectory-averaged error is therefore not comparable with these $\alpha$-$\lambda$ scores, and a downstream
user should read a bearing RUL number with this uncertainty.

\begin{table}[!t]
\centering
\caption{RUL under the Saxena $\alpha$-$\lambda$ protocol ($\alpha=0.2$) over $23$ real run-to-failure
trajectories. Higher is better; all three models score low.}
\label{tab:rul}
\footnotesize
\begin{tabular}{@{}l c c c@{}}
\toprule
Model & \begin{tabular}[b]{@{}c@{}}$\alpha$-$\lambda$\\accuracy\end{tabular}
      & \begin{tabular}[b]{@{}c@{}}cum. rel.\\accuracy\end{tabular}
      & \begin{tabular}[b]{@{}c@{}}\#\\predictions\end{tabular} \\
\midrule
...
Two findings stand, and they contrast sharply. \emph{Diagnostics on CWRU is largely solved when evaluated
without leakage}: a learned classifier reaches perfect held-out-load accuracy with graceful noise degradation,
and even a training-free envelope method handles three of four fault classes, failing on the fourth for an
identifiable physical reason. \emph{Prognostics on real run-to-failure bearings is not solved at the agreed metric}: no model in
a reasonable ladder places more than about one prediction in twenty inside the $\pm20\%$ cone. The asymmetry
means that a reported prognostics accuracy has to be read with its metric ($\alpha$-$\lambda$ or
trajectory-averaged) and its leakage control, and that a bearing RUL estimate is best used today as a wide,
monotone trend with a large uncertainty band, not a point number to schedule against. The
benchmark's contribution is to make both statements reproducible: a leakage-immune diagnostics protocol and the
$\alpha$-$\lambda$ prognostics metric, run over the standard data, with the artifacts archived so that every number
can be checked.

\section{Scope and Limitations}\label{sec:limits}

The diagnostics evaluation is on CWRU seeded-fault data; seeded faults are cleaner than
...
aim here is the classical baseline under the accepted metric. The health indicator is a scalar RMS-type amplitude; richer
multi-feature HIs exist and are future work. The datasets are the public standards, referenced and linked but not
re-hosted per their terms. The interactive study states the same scope on every panel.
...
$\alpha$-$\lambda$ accuracy $0.048$). The contribution is a reproducible, leakage-immune benchmark that reports
both, with the artifacts archived and the raw data referenced rather than re-hosted, and that tells a downstream
user what to trust and what not to.

\appendices
\section{Reproduction}\label{app:repro}

{\raggedright
All numbers come from artifacts archived in the repository under \path{data/derived/}: the CWRU diagnostics
benchmark (\path{cwru-benchmark.json}), the learned-model metrics (\path{rv-learned-metrics.json}), and the RUL
benchmark (\path{rv-rul-benchmark.json}) with its $23$ trajectories and Saxena checkpoints. The figure script
reads those artifacts; the method illustration uses an archived controlled-degradation trace. The datasets are
public and are referenced (CWRU~\cite{smith}, XJTU-SY~\cite{wang}, FEMTO~\cite{nectoux}, IMS~\cite{qiu}); the raw
recordings are linked, never re-hosted.\par}
