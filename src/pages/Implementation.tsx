      ...
      content: (
        <div className="prose">
          ...
          <p>{es
            ...
            : 'This single primitive is the workhorse beneath the band-pass, the Hilbert transform, the cepstrum, the spectrogram, and the cyclic-modulation spectrum. Building everything on one verified O(N log N) kernel keeps the live tier auditable and fast.'}</p>
      ...
      content: (
        <div className="prose">
          ...
          <p>{es
            ...
            : 'Where it works: outer-race faults at moderate SNR, where the impact train is strong and stationary. Where it fails: the wrong band gives a flat, ambiguous spectrum — band selection is the single hardest decision, which is exactly why the kurtogram exists. Inner-race and ball faults are intrinsically weaker (extra transmission path and speed modulation) and smear under slip.'}</p>
      ...
      content: (
        <div className="prose">
          <p>{es
            ...
            : 'The real cepstrum is the inverse FFT of the log-magnitude spectrum; a uniformly spaced family of spectral lines (a harmonic train or sideband family) collapses to a single peak at the quefrency equal to their spacing-s reciprocal. It is the natural detector for "is there a regularly spaced family here?" and complements the spectrum.'}</p>
          <Equation
          ...
          <p>{es
            ...
            : 'A second-order cyclostationary fault appears as a vertical α-ridge family at the fault frequency and its harmonics, independent of the carrier band — the cleanest "is this periodic at the fault period, anywhere in the spectrum?" view.'}</p>
          <Callout variant="note" title={es ? 'Aproximación, no referencia' : 'Approximation, not gold standard'}>
            {es
              ...
              : 'The build is explicit that the heavier, gold-standard cyclic-spectral-correlation map is a precompute method, not this live approximation. The live view prioritizes in-browser feasibility over full fidelity.'}
          </Callout>
      ...
      content: (
        <div className="prose">
          <p>{es
            ...
            : 'The verdict is not a black box. For each candidate fault (outer→BPFO, inner→BPFI, ball→2·BSF) the engine scores the prominence of that line in the envelope spectrum: for each of the first five harmonics it takes the local peak within a tolerance window and divides it by the median of the surrounding bins (a robust local baseline that ignores the peak itself), then averages over the harmonics that fit below the spectrum-s top.'}</p>
          <p>{es
      ...
      content: (
        <div className="prose">
          ...
          <p>{es
            ...
            : 'The threshold-crossing time is solved in closed form, the remaining useful life is failTime − t_last, and a forward ±2σ_residual fan is drawn from the regression-s log-scale residual spread, so the projection is reported as a band, not a single deceptive line.'}</p>
          <Equation
      ...
      content: (
        <div className="prose">
          <p>{es
            ...
            : 'Overall vibration severity is mapped onto the standard A/B/C/D evaluation zones, giving a familiar at-a-glance band (good / acceptable / unsatisfactory / unacceptable) alongside the detailed spectra.'}</p>
          <p>{es
          ...
          <p>{es
            ...
            : 'Because this is the exact inverse of what the envelope-analysis chain detects, the demos close the loop: the model plants a known fault frequency and the live engine recovers it, which is the honest meaning of "validation set" here.'}</p>
