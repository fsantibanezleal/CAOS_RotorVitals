      ...
      content: (
        <div className="prose">
          ...
          <p>{es
            ...
            : 'These are different physics and different failure modes: a method can be excellent at diagnosis on strong faults and useless at early detection, or accurate on a held-out load and badly miscalibrated on a new one. Reporting them apart is the honest minimum.'}</p>
      ...
      content: (
        <div className="prose">
          <p>{es
            ...
            : 'The predicted class is the argmax of the prominence score over three candidates: outer race at BPFO, inner race at BPFI, and rolling element at 2·BSF — the doubled ball-spin frequency, because a ball defect strikes both races per revolution so its dominant envelope line is the second harmonic of BSF.'}</p>
          <Equation
      ...
      content: (
        <div className="prose">
          ...
          <p>{es
            ...
            : 'This is grounded directly in Smith & Randall’s benchmark grading of records, which showed that several inner-race and ball records are genuinely difficult even with an optimal envelope pipeline — so a method scoring ~100 % on those records is almost certainly leaking, not winning.'}</p>
      ...
      content: (
        <div className="prose">
          ...
          <p>{es
            ...
            : 'What is illustrative only is the severity → onset → RUL mapping: the run-to-failure health-indicator trend is a synthetic, realistically-shaped curve where higher severity drives an earlier degradation onset and faster exponential growth. The shape is realistic; the specific hours are not a measurement.'}</p>
      ...
      content: (
        <div className="prose">
          ...
          <p>{es
            ...
            : 'A healthy trajectory shows no sustained onset, so the model correctly returns no projection rather than a fabricated number. The error metric is the gap between this projected RUL and the trajectory’s true failure time. The exponential-then-first-passage form is the standard threshold-crossing prognostic family; ISO 20816 severity zones supply a physically meaningful alarm threshold when one is not dataset-defined.'}</p>
