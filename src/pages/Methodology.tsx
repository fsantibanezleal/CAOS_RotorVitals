...
export default function Methodology() {
  ...
  const mlTab = (
    <div className="prose">
      <p>{es
        ? 'Sobre features (RMS, kurtosis, factores de cresta/impulso, entropía) clasificadores clásicos (SVM/RF/XGBoost); o deep learning de extremo a extremo: 1D-CNN (WDCNN), 2D-CNN sobre escalogramas/espectrogramas, LSTM, y adaptación de dominio para generalizar entre cargas/máquinas. El entrenamiento es offline; la inferencia en el navegador es vía un runtime ONNX. El valor de las features físicas (las mismas RMS/kurtosis/prominencia de las pestañas anteriores) es que el modelo parte de cantidades ya interpretables, no de píxeles crudos.'
        : 'Over features (RMS, kurtosis, crest/impulse factors, entropy) classical classifiers (SVM/RF/XGBoost); or end-to-end deep learning: 1D-CNN (WDCNN), 2D-CNN on scalograms/spectrograms, LSTM, and domain adaptation to generalize across loads/machines. Training is offline; in-browser inference runs through an ONNX runtime. The value of physical features (the same RMS/kurtosis/prominence from the earlier tabs) is that the model starts from already-interpretable quantities, not raw pixels.'}</p>
      <Callout variant="honest" title={es ? 'Honestidad de evaluación' : 'Evaluation honesty'}>
        <p>{es
          ? 'Se reporta la partición (sin fuga del conjunto de referencia) y la prueba cruzada de carga; la exactitud sin esas salvaguardas es engañosa. El deep learning profundo queda fuera del alcance de esta página, que se centra en los métodos de procesamiento de señal que el build implementa en vivo.'
          : 'Report the split (no leakage of the reference set) and the cross-load test; accuracy without those safeguards is misleading. Deep DL is out of scope for this page, which centers on the signal-processing methods the build implements live.'}</p>
      </Callout>
