...
export function FeatureSpacePanel({ bearing, fault, severity, snr, rpm, lifeH, lang }: {
  ...
  const note = es
    ? 'Cada punto es una instantánea de vida (color = vida). La elipse es la novedad 95% (Mahalanobis) del cúmulo sano; el caso con falla se aleja de ella tras el onset, mientras un caso sano permanece dentro. Features hechas a mano (RMS, curtosis, amplitud de defecto SES) — son la línea base honesta que un espacio latente aprendido por autoencoder profundo mejora (siguiente iteración).'
    : 'Each point is a life snapshot (color = life). The ellipse is the 95% Mahalanobis novelty of the healthy cluster; the fault case departs it after onset while a healthy case stays inside. Hand-crafted features (RMS, kurtosis, SES defect amplitude) — the honest baseline that a learned deep-autoencoder latent space improves on (next iteration).';
