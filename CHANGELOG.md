...
### Changed
- Content standards (ADR-0067): removed every em-dash from tracked content (replaced with commas /
  "n/a" in tables) and the printer emoji from the RUL recommendation Print button. No behaviour change.
- Added `scripts/check_content_standards.py` and wired it into the CI `guards` job so the repo cannot
  regress: the build now fails on any em-dash or pictographic emoji.
