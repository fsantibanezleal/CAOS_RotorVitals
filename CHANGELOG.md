...
Reviewing the prognostics lane in detail against the shipped artifacts rather than the plan records.

- **The deep-RUL CNN was the only model never measured.** `evaluate_rul.py` documents that it "is skipped
  unless its ONNX exists", but the model list was hardcoded to `["exponential", "pf", "gp"]`, so the one
  model that ships trained weights (`deep_rul.onnx`, XJTU-SY + FEMTO) was excluded from the benchmark while
  remaining selectable in the App. It is now included whenever the ONNX is present, under the identical
  Saxena checkpoint protocol. Whether it scores well is a separate question; being unmeasured beside
  measured models is the asymmetry this removes. The published numbers still read `pending` until the heavy
  pipeline lane is rerun, which the Benchmark page already states.
- **A score from one prediction was rendered like a score from twenty-one.** The Gaussian Process showed
  `0.0%` α-λ accuracy and `0.000` CRA from a SINGLE evaluable checkpoint prediction, in the same column and
  formatting as the exponential model's 21. That reads as "evaluated and worst" when the truth is
  "essentially unevaluated". Below five predictions the row now reads **insufficient**, and the honest-
  reading paragraph says why. The threshold is applied by the page from `n_predictions` rather than from a
  flag in the artifact, because the artifact is regenerated by the pipeline and a flag alone would silently
  revert to printing zeros as results.

Neither change alters a model or a number. Both change what the app CLAIMS to have measured, which is the
part that was wrong.
