...
\text{RUL} = t_{\text{fail}} - t_{\text{last}}`}
        ...
        <p>{es
          ? 'Este modelo exponencial es el primer escalon de un ladder de pronostico que ahora incluye tres herramientas adicionales, todas implementadas en ambas lineas (TypeScript navegador + Python pipeline): un filtro de particulas Bayesiano (500 particulas, remuestreo sistematico, distribucion posterior completa del RUL), un Proceso Gaussiano con kernel compuesto RBF+Matern(5/2)+WhiteNoise optimizado via L-BFGS-B (scikit-learn en el pipeline, Cholesky en TypeScript), y una CNN profunda (Deep-RUL, backbone WDCNN) entrenada sobre XJTU-SY+FEMTO, exportada a ONNX e inferida en vivo con onnxruntime-web. La documentacion de metodologia cubre los cuatro modelos con referencias: An & Choi (2013) para PF, Rasmussen & Williams (2006) y Liu et al. (2020) para GP, Li, Ding & Sun (2018) y Zhu, Chen & Peng (2019) para Deep-RUL.'
