  ...
  return (
    <div className="page-body prose">
      ...
      <section>
        ...
        <p>{es
          ? 'El resultado es una portadora (la resonancia) modulada en amplitud a la baja frecuencia de defecto cinemática (la tasa de contacto). En el espectro crudo la energía está arriba, en la resonancia, difuminada por toda otra fuente de banda ancha, y la lenta tasa de defecto queda enterrada como bandas laterales de modulación fáciles de pasar por alto. La falla se esconde en cómo se modula la energía de alta frecuencia, no en una línea aislada. Por eso justamente la demodulación de amplitud (análisis de envolvente) es la herramienta correcta: quitar la portadora y recuperar la señal moduladora expone la tasa de defecto como una línea espectral limpia, allí donde el espectro crudo no mostraba nada accionable.'
          : 'The result is a carrier (the resonance) amplitude-modulated at the low kinematic defect frequency (the contact rate). In the raw spectrum the energy sits up at the resonance, smeared by every other broadband source, and the slow defect rate is buried as modulation sidebands that are easy to miss. The fault hides in how the high-frequency energy is modulated, not in any single line. That is precisely why amplitude demodulation (envelope analysis) is the correct tool: stripping away the carrier and recovering the modulating signal exposes the defect rate as a clean spectral line, where the raw spectrum showed nothing actionable.'}{' '}<Cite id="randall2011" paren /></p>
        <Refs ids={['randall2011']} label={es ? 'Refs' : 'Refs'} />
      </section>
...
\text{RUL} = t_{\text{fail}} - t_{\text{last}}`}
        ...
        <p>{es
          ? 'Esta lógica de primer cruce es un método pronóstico real y estándar. Las relaciones de procesamiento de señal son exactas, las frecuencias cinemáticas, la demodulación y la lógica de prominencia armónica se transfieren directamente a vibración real medida. El App tiene un selector de fuente: en modo real opera sobre datos medidos (segmentos de diagnóstico CWRU/Ottawa/MaFaulDa; trayectorias run-to-failure FEMTO/XJTU/IMS, cada una con sus frames crudos de vida y su tiempo de falla real del experimento). En modo sintético, las severidades y trayectorias son sintéticas y etiquetadas como tales: el generador construye un tren de impulsos de resonancia amortiguada con base física (cada impulso una resonancia decreciente '
          : 'This first-passage logic is a real, standard prognostic method. The signal-processing relations are exact, the kinematic frequencies, the demodulation, and the harmonic-prominence logic transfer directly to real measured vibration. The App has a source selector: in real mode it operates on measured data (CWRU/Ottawa/MaFaulDa diagnosis segments; FEMTO/XJTU/IMS run-to-failure trajectories, each with its raw life-frames and the experiment\'s real failure time). In synthetic mode the severities and trajectories are synthetic and labeled as such: the generator builds a physically-grounded damped-resonance impulse train (each impulse a decaying '}
        ...
        <p>{es
          ? 'Este modelo exponencial es el primer escalón de una escalera de pronóstico que ahora incluye tres herramientas adicionales, todas implementadas en ambas líneas (TypeScript navegador + Python pipeline): un filtro de partículas Bayesiano (500 partículas, remuestreo sistemático, distribución posterior completa del RUL), un Proceso Gaussiano con kernel compuesto RBF+Matérn(5/2)+WhiteNoise optimizado vía L-BFGS-B (scikit-learn en el pipeline, Cholesky en TypeScript), y una CNN profunda (Deep-RUL, backbone WDCNN) entrenada sobre XJTU-SY+FEMTO, exportada a ONNX e inferida en vivo con onnxruntime-web. La documentación de metodología cubre los cuatro modelos con referencias: An & Choi (2013) para PF, Rasmussen & Williams (2006) y Liu et al. (2020) para GP, Li, Ding & Sun (2018) y Zhu, Chen & Peng (2019) para Deep-RUL.'
          : 'This exponential model is the first rung of a prognostic ladder that now includes three additional tools, all implemented in both lanes (TypeScript browser + Python pipeline): a Bayesian Particle Filter (500 particles, systematic resampling, full posterior RUL distribution), a Gaussian Process with composite RBF+Matern(5/2)+WhiteNoise kernel optimised via L-BFGS-B (scikit-learn in the pipeline, Cholesky in TypeScript), and a Deep-RUL CNN (WDCNN backbone) trained on XJTU-SY+FEMTO, exported to ONNX and inferred live via onnxruntime-web. The methodology documentation covers all four models with references: An & Choi (2013) for PF, Rasmussen & Williams (2006) and Liu et al. (2020) for GP, Li, Ding & Sun (2018) and Zhu, Chen & Peng (2019) for Deep-RUL.'}</p>
