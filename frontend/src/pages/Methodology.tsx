  ...
  const mlTab = (
    <div className="prose">
      ...
      <Callout variant="honest" title={es ? 'Honestidad de evaluación' : 'Evaluation honesty'}>
        <p>{es
          ? 'Se reporta la partición (sin fuga del conjunto de referencia) y la prueba cruzada de carga (se deja FUERA una carga entera, 3 HP); la exactitud sin esas salvaguardas es engañosa. El WDCNN + el autoencoder profundo SÍ están implementados y corren en vivo en la página Benchmark (diagnóstico interactivo sobre segmentos reales + los números held-out); CWRU es un banco limpio, por eso se reporta la degradación honesta vs ruido en vez de un 100% pelado.'
          : 'Report the split (no leakage of the reference set) and the cross-load test (an ENTIRE load, 3 HP, is held out); accuracy without those safeguards is misleading. The WDCNN + deep autoencoder ARE implemented and run live on the Benchmark page (interactive diagnosis on real segments + the held-out numbers); CWRU is a clean lab rig, so we report the honest noise-degradation curve rather than a bare 100%.'}</p>
      </Callout>
