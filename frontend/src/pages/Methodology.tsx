...
export default function Methodology() {
  ...
  const decompTab = (
    <div className="prose">
      ...
      <p>{es
        ? 'Descomposición I, la familia de modo empírico (EMD/EEMD/CEEMDAN). EMD es puramente adaptativa: tamiza la señal interpolando envolventes de spline cúbico por los máximos y mínimos locales, restando su media e iterando hasta una Función de Modo Intrínseco (IMF), un componente con envolvente simétrica e igual número de extremos que de cruces por cero. La IMF que porta la falla es la de mayor curtosis, el mismo criterio de impulsividad del kurtograma. El EMD simple sufre mezcla de modos: un impulso intermitente puede partirse entre IMFs. EEMD lo corrige por asistencia de ruido, agrega muchas realizaciones de ruido blanco, ejecuta EMD en cada una y promedia. CEEMDAN agrega el ruido de forma adaptativa, etapa por etapa, y resta el residual del ensemble en cada paso. El costo honesto: se ejecutan decenas de pasadas iterativas, no reproducibles bit-a-bit entre implementaciones, sin teoría cerrada; por eso esta familia es un artefacto precomputado/offline, no una transformada en vivo.'
  ...
  const mlTab = (
    <div className="prose">
      ...
      <p>{es
        ? 'El resultado held-out es honesto y revelador: el WDCNN profundo llega a 100%, mientras el SVM-RBF y el Random Forest se quedan en ~85.6%. La columna que lo explica es el "recall de la clase sana": el ML clásico clava las fallas (externa/interna ~100%, bola ~90%) pero falsa-alarma en la mitad de las ventanas sanas, las prominencias de los peines, hechas a mano, también disparan en señales sanas con transitorios. El CNN profundo aprende esa frontera sano/falla que las features fijas no capturan. No es una victoria fabricada: el split es idéntico, los tres se ejecutan en vivo sobre los mismos segmentos, y el número se reporta como cae.'
