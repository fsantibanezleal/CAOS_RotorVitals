  ...
  const ArchSVG = (
    ...
    >
      ...
      <desc id="impl-arch-desc">
        {es
          ? 'Una vía pesada offline ejecuta entrenamiento, barridos de parámetros y pronósticos, y congela el resultado en un artefacto compacto y determinista. Una vía delgada en vivo en el navegador ejecuta DSP acotado sobre un segmento de señal. Ambas están sembradas y son reproducibles; los archivos estáticos se sirven desde un CDN.'
      ...
      <text x="728" y="372" fontSize="10.5" fill="var(--color-fg)">{es ? 'ejecuta el DSP en vivo local' : 'run live DSP locally'}</text>
  ...
  const tabs = [
    {
      ...
      content: (
        <div className="prose">
          ...
          <p>{es
            ? 'En cambio, el build traza una línea dura: los métodos cuyo costo es acotado y suficientemente pequeño para ejecutarse interactivamente sobre un segmento se implementan en vivo; los métodos cuya forma honesta exige registros largos, solucionadores iterativos o barridos grandes se calculan fuera de línea y se entregan como el resultado de ese cómputo, nunca re-derivados en la página.'
    ...
    },
    {
      ...
      content: (
        <div className="prose">
          <p>{es
            ? 'El producto se entrega como activos estáticos pre-construidos, HTML, JavaScript, los artefactos compactos precalculados, servidos directamente desde una red de distribución de contenidos (CDN), sin servidor de aplicación ni base de datos en la ruta de la solicitud. El navegador descarga el bundle una vez y ejecuta el DSP en vivo localmente; el único «backend» es la caché de borde del CDN.'
  ...
  return (
    <div className="page-body">
      <div className="page-head prose">
        ...
        <p className="lede">{es
          ? 'Cómputo pesado offline → artefacto compacto → runtime fino. Lo liviano se ejecuta en vivo en el navegador, sembrado y reproducible.'
