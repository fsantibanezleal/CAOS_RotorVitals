    ...
    // ----------------------------------------------------------------- PROGNOSIS
    {
      ...
      content: (
        <div className="prose">
          ...
          />
          <p>{es
            ? 'Una trayectoria sana no muestra inicio sostenido, así que el modelo correctamente devuelve ninguna proyección en vez de un número fabricado. En el App, la proyección se contrasta por trayectoria contra el tiempo de falla real medido (pestaña RUL), y la pestaña Eval RUL calcula la curva α-λ en vivo. La forma exponencial-luego-primer-cruce es la familia estándar de pronóstico por cruce de umbral; las zonas de severidad ISO 20816 aportan un umbral de alarma físicamente significativo cuando el dataset no lo define.'
            : 'A healthy trajectory shows no sustained onset, so the model correctly returns no projection rather than a fabricated number. In the App, the projection is checked per trajectory against the measured true failure time (RUL tab), and the RUL Eval tab computes the α-λ curve live. The exponential-then-first-passage form is the standard threshold-crossing prognostic family; ISO 20816 severity zones supply a physically meaningful alarm threshold when one is not dataset-defined.'}{' '}<Cite id="iso20816" paren /></p>
          <p>{es
            ? 'Tres modelos adicionales repiten el experimento sobre las mismas trayectorias. El filtro de partículas (500 partículas, remuestreo sistemático con regularización kernel) estima la distribución posterior completa del RUL; la métrica de error se toma sobre la mediana. El proceso Gaussiano (scikit-learn GaussianProcessRegressor, kernel RBF+Matérn(ν=2.5)+WhiteKernel, optimización L-BFGS-B) evalúa la incertidumbre calibrada. La CNN-BiLSTM (Deep-HI/RUL) procesa las ventanas de vibración cruda como secuencia temporal y emite la curva HI proyectada directamente. De las 36 trayectorias, 23 tienen una falla real por primer cruce y son evaluables; la consolidación offline en un benchmark comparativo está en re-evaluación, el protocolo agregado original resultó defectuoso y sus números fueron retirados (issue #128). La página Benchmark documenta el estado.'
            : 'Three additional models repeat the experiment on the same trajectories. The particle filter (500 particles, systematic resampling with kernel regularisation) estimates the full posterior RUL distribution; the error metric is taken on the median. The Gaussian Process (scikit-learn GaussianProcessRegressor, RBF+Matérn(ν=2.5)+WhiteKernel, L-BFGS-B optimisation) evaluates calibrated uncertainty. The CNN-BiLSTM (Deep-HI/RUL) processes raw vibration windows as a temporal sequence and outputs the projected HI curve directly. Of the 36 trajectories, 23 have a real first-passage failure and are evaluable; the offline consolidation into a comparative benchmark is under re-evaluation, the original aggregate protocol was found flawed and its numbers were withdrawn (issue #128). The Benchmark page documents the status.'}{' '}
            <Cite id="aye2017" paren />
          </p>
          <Refs ids={['lei2018', 'iso20816', 'wang2020xjtu', 'aye2017', 'an2015']} label={refsLabel} />
        </div>),
    },
  ];
