            ...
            <table className="cmp-table" style={{marginTop:'0.75rem'}}>
              ...
              <tbody>
                {([
                  ...
                  const m = (rulBench.summary.models as Record<string, { alpha_lambda_accuracy: number | null; cra: number | null; n_predictions: number; insufficient?: boolean }>)[id];
                  const pct = (v: number | null) => v == null ? '-' : `${(v * 100).toFixed(1)}%`;
                  // A score from one or two evaluable trajectories is an artefact of WHICH trajectories
                  // happened to be evaluable, not a measurement. GP scored 0.0% / 0.000 from a SINGLE
                  // prediction and was rendered exactly like a model scored over 21, which reads as
                  // "evaluated and worst" when the truth is "essentially unevaluated". Say so instead.
                  // Derived from n, not from a flag in the artifact: the artifact is regenerated by the
                  // pipeline, so a hand-patched flag would not survive and the UI would quietly go back to
                  // printing 0.000 as if it were a result. The threshold lives with the consumer that has
                  // to render the claim. `insufficient` is honoured when the pipeline does emit it.
                  const MIN_PREDICTIONS = 5;
                  const thin = m?.insufficient === true || (m?.n_predictions ?? 0) < MIN_PREDICTIONS;
                  return (
                    <tr key={id}>
                      <td style={{textAlign:'left'}}>{label}</td>
                      <td className="mono">{thin ? <span className="muted" title={es ? 'muestras insuficientes para un puntaje agregado' : 'too few predictions for an aggregate score'}>{es ? 'insuficiente' : 'insufficient'}</span> : pct(m?.alpha_lambda_accuracy)}</td>
                      <td className="mono">{thin ? <span className="muted">-</span> : (m?.cra == null ? '-' : m.cra.toFixed(3))}</td>
