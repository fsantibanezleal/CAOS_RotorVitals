...
function SesPlot({ ses, f, fr }: { ses: { freq: Float64Array; mag: Float64Array }; f: ReturnType<typeof defectFreqs>; fr: number }) {
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} width="100%" role="img" aria-label="envelope spectrum" style={{ font: '10px var(--font-mono)' }}>
      ...
      <text x={W - padR} y={H - 6} textAnchor="end" fill="var(--color-fg-faint)">{fmax.toFixed(0)} Hz · envelope spectrum (supplied data)</text>
    </svg>
  );
}
...
export function IngestPanel({ lang }: { lang: 'en' | 'es' }) {
  ...
  const onExample = () => {
    ...
    setText(es ? '(ejemplo sintético de 12000 muestras cargado, pegar o subir datos propios para análisis real)' : '(synthetic 12000-sample example loaded, paste or upload supplied data for real analysis)');
    setResult(analyze(sig.x, exFs, exRpm, bearingById(bId), lang));
    setErr(null); setInfo(es ? 'ejemplo sintético de pista-externa (1 s @ 12 kHz, etiquetado); para datos reales, subirlos arriba' : 'synthetic outer-race example (1 s @ 12 kHz, labelled), for real data, upload it above');
  };

  const rec = result?.rec;
  return (
    <section>
      <h2>{es ? 'Análisis de datos propios' : 'Bring your own data'}</h2>
      <p className="muted small">{es
        ? 'Pegar o subir una señal de vibración (un número por línea, o CSV, se toma la última columna numérica), indicar la tasa de muestreo, la velocidad de eje y la geometría del rodamiento, y ejecutar el pipeline real: kurtograma → banda → envolvente/SES → diagnóstico → recomendación (la capa T5). La física no-supervisada es agnóstica al banco, así que funciona en cualquier rodamiento.'
