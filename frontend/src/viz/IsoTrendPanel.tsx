...
import { lifeFeatures, ISO_CLASS_I, type IsoBounds } from '../dsp/iso';
import { type FaultKind, type Bearing } from '../dsp/bearing';
import { UPlotChart } from './UPlotChart';
import { lineOpts, hbandsPlugin, hlinesPlugin } from './uplotKit';

/** ISO broadband velocity-RMS severity trend over the run-to-failure life, with the selected ISO scale's
 * zone bands (A/B/C/D) and the ALERT (B/C) / DANGER (C/D) setpoint lines (T7, class/group is configurable).
...
export function IsoTrendPanel({ bearing, fault, severity, snr, rpm, lifeH, bounds, isoLabel, lang }: {
  bearing: Bearing; fault: FaultKind; severity: number; snr: number; rpm: number; lifeH: number;
  bounds?: IsoBounds; isoLabel?: string; lang: 'en' | 'es';
}) {
  const es = lang === 'es';
  const b = bounds ?? ISO_CLASS_I;
  ...
  const vmax = useMemo(() => Math.max(b.cd * 1.08, ...feats.map((f) => f.vrms)) * 1.05, [feats, b]);
