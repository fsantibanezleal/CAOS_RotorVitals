...
export function GramPanel({ x, fs, onPick, lang, f, fr, faultAlpha }: {
  ...
  const note = es
    ? 'El kurtograma puntúa cada banda por la kurtosis de la envolvente; el infograma (Antoni 2016) por la negentropía de la envolvente al cuadrado (SE) y de su espectro (SES), sensible a transitorios repetitivos. El IESFOgrama (Mauricio et al. 2020) puntúa cada banda no por impulsividad general sino por cuán fuerte muestra su SES el peine de armónicos de la frecuencia de falla diagnosticada (BPFO/BPFI/2·BSF), un cociente adimensional (× sobre la línea base local). Activar "spike" (impulsos no repetitivos): la mejor celda del kurtograma salta a la banda del spike, la del SES se mueve a lo sumo una celda, y la del IESFOgrama dirigido queda igual. La variante ciega barre órdenes de eje (≥1.5×fr) y es más débil. Clic en una celda fija la banda → SES en vivo. (Datos sintéticos.)'
